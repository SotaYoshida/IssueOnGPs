...
function TFplot(Vs)
    ...
    for i=1:3
        ...
        axs[i].set_ylabel("\$ \\log_{10}\\sigma \$ ")
    end
    axs[1].set_title("RBF")
    axs[2].set_title("Mat52");axs[3].set_title("Mat32")
    for V in Vs
...
function difplot(difs)
    ...
    for i =1:length(axs)
        ...
        r = patches.Rectangle(xy=(-6, -6), width=12.0, height=7.0, color="gray", fill=true,zorder=0,alpha=0.4)
        axs[i].add_patch(r)
...
function main()
    ...
    #xt = log.([6.0,8.0,10.0,12.0,14.0])
    oyt= [-28.602,-30.213,-31.176,-31.713,-31.977]

    meany = mean(oyt)
    muy=0*oyt .+ meany; sigy=std(oyt)
    yt =(oyt - muy)/sigy
    xp = collect(16.0:2.0:20.0)
    #xp = collect(16.0:2.0:30.0)
    ...
    taus   = [ i * 10.0^j for j=-5:1:5 for i=1:2:5]
    sigmas = [ i * 10.0^j for j=-5:1:5 for i=1:2:5]

    difs_R=[];difs_M=[];difs_M3=[]
    Vs=[]
    for tau in taus
        for sigma in sigmas
            ...
            if PSD_R != 3.0           
                ...
                logObsNoise = -1.e+305
                gpR = GP(xt,yt,mZero,kRBF,logObsNoise)
                try 
                    R_mu,R_S=predict_y(gpR,xp)
            ...
            if PSD_M != 3.0 
                ...
                logObsNoise = -1.e+305
                gpM = GP(xt,yt,mZero,kMat52,logObsNoise)
                try 
                    M_mu,M_S=predict_y(gpM,xp)
            ...
            if PSD_3 != 3.0 
                ...
                logObsNoise = -1.e+305
                gpM3 = GP(xt,yt,mZero,kMat32,logObsNoise)
                try 
                    M3_mu,M3_S=predict_y(gpM3,xp)
